...
abstract class RestServer {
   ...
   * @see dispatch()
   */
  public function rawDispatch($command, array $source = null) {
    if ( !isset($this->actions[$command]) )
      throw new PageNotFoundException('Unknown command: '.$command);
   ...
   * @return void
   */
  public function runJSON($command) {
    $res = $this->dispatch($command);
    if ( $res != null ) {
